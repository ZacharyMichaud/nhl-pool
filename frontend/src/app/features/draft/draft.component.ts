...
@Component({
  selector: 'app-draft',
  standalone: true,
  imports: [CommonModule, FormsModule, MatFormFieldModule, MatInputModule, MatSelectModule, MatButtonModule, MatSnackBarModule],
...
export class DraftComponent implements OnInit {
  ...
  getStat(p: any, col: SortColumn): string {
    ...
    if (col === 'toi') return p.regularSeasonAvgToi || '-';
    return '' + val;
  }
  ...
  getDisplayStat(p: any, col: SortColumn): string {
    ...
    if (col === 'ppg') return this.formatDecimal(raw);  // P/GP is always per-game
  ...
  private getRawStat(p: any, col: SortColumn): number {
    ...
    switch (col) {
      ...
      case 'toi': return this.toiToMinutes(p.regularSeasonAvgToi);
      default: return 0;
    }
  }
